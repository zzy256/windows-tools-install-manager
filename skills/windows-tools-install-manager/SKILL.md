...
description: Use when installing a system-level software tool, CLI binary, GUI app, utility, OCR engine, video downloader, font, archive tool, or other end-user program on this Windows machine — both when the user explicitly asks ("install X", "装/安装/装个/装一下/搞一个 X") AND when you discover mid-task that a required system tool isn't installed (e.g., tesseract for OCR, ffmpeg for video, 7zip for extraction, gh CLI for GitHub). Install root is user-configurable (default suggestion D:\Tools\<tool-name>\). Always pause and confirm the install plan with the user before running anything. Do NOT use for: Python packages (→ miniconda-python-env), Node/npm packages inside a project, Claude Code / Codex / Cursor themselves, OS components / drivers / Windows Updates, code compilation tasks ("compile this C++", "编译一下"), read-only questions about a tool ("what does X do", "explain Y", "X vs Y", "哪个更好/哪个快"), looking up where an already-installed tool lives ("X 装在哪", "where is X installed"), cases where the user states the tool is already installed (just use it), uninstall / delete / remove operations on existing tools ("删了 X", "卸载 X"), VS Code / browser extensions (those have their own install UI), or Steam / game installs.
---
