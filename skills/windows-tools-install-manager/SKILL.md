...
description: Use when installing a SYSTEM-LEVEL software tool, CLI binary, GUI app, utility, OCR engine, video downloader, font, archive tool, or compiled program on this Windows machine — both when the user explicitly asks ("install X", "装/安装/装个/装一下/搞一个 X") AND when you discover mid-task that a required system tool isn't installed (e.g., tesseract for OCR, ffmpeg for video, 7zip for extraction, gh CLI for GitHub). Standard install root is configurable (default suggestion D:\Tools\<tool-name>\; resolved per-user via Step 0). Always pause and confirm the install plan with the user before running anything. Do NOT use for: Python packages (numpy, pandas, requests, fastapi, etc. — those go to miniconda-python-env), Node/npm packages inside a project, Claude Code or Codex themselves, OS components/drivers/Windows updates, read-only questions about a tool ("what does X do", "explain Y", "X vs Y", "哪个更好/哪个快"), cases where the user states the tool is already installed (just use it instead of installing it), or uninstall/delete/remove operations on existing tools ("删了 D:\Tools 里的 X", "卸载 X").
---
